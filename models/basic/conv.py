...
class Conv2d(nn.Module):  # 包装好的2D卷积模块，由多个层组成，可以根据需要实现普通2D卷积或者深度可分离2D卷积
    def __init__(self, 
                 c1,                   # in channels
                 c2,                   # out channels 
                 k=1,                  # kernel size 
        ...
        if depthwise:  # 深度可分离卷积可以分为先depthwise conv再pointwise conv，即通道不变尺寸变(通道之间独立)，再尺寸不变通道变
...
class Conv3d(nn.Module):  # 包装好的3D卷积模块，由多个层组成，可以根据需要实现普通3D卷积或者深度可分离3D卷积
    def __init__(self, 
                 c1,                   # in channels
                 c2,                   # out channels 
                 k=1,                  # kernel size 

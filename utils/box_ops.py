...
def calculate_iou(box1, box2, xywh=False, iou_type='iou', Focal=0, alpha=1, gamma=0.5, scale=False, eps=1e-7,
                  img_size=224):
    """
        Compute iou 支持0~1输入或者绝对坐标输入
        Args:
            box1 (tensor): pred values   shape(n,4)
            box2 (tensor): target values  shape(n,4)
            xywh (bool): True or False
            iou_type (str): 'iou', giou', 'diou', 'ciou', 'eiou', 'siou', 'riou', 'wiou, 'riou', 'mpdiou'
            ...
            eps (float): 避免为0的极小数
            img_size (int): 224  # 输入图片的对角线的平方,对应绝对坐标输入
    ...
    # IoU
    iou = inter / (union + eps)
    if iou_type in ['giou', 'diou', 'ciou', 'eiou', 'siou', 'riou', 'wiou', 'mpdiou', 'shapeiou']:
        cw = b1_x2.maximum(b2_x2) - b1_x1.minimum(b2_x1)  # convex (smallest enclosing box) width   最小外接矩形的宽度
        ch = b1_y2.maximum(b2_y2) - b1_y1.minimum(b2_y1)  # convex height
        if iou_type == 'giou':  # GIoU -1~1 https://arxiv.org/pdf/1902.09630.pdf
            ...
            if iou_type == 'diou':  # DIoU -1~1
                if Focal != 0:
                    gradient_gain = (1 - iou).pow(gamma) if Focal == 1 else iou.pow(gamma)
                    return iou.pow(alpha) - (rho2 / (c2 + eps)).pow(alpha), gradient_gain  # Focal_DIoU
                else:
                    return iou.pow(alpha) - (rho2 / (c2 + eps)).pow(alpha)  # DIoU
            elif iou_type == 'ciou':  # CIoU -1~1 https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
...
if __name__ == '__main__':
    box1 = torch.tensor([497, 364, 719, 999]).unsqueeze(0)
    box2 = torch.tensor([492, 357, 715, 999]).unsqueeze(0)
    ...
    print("MPDIoU有交集：", calculate_iou(box1, box2, iou_type='mpdiou'))
    print("=" * 20)
    ...
    print("MPDIoU无交集：", calculate_iou(box1, box3, iou_type='mpdiou'))

...
# 无视类别的多类别nms
def multiclass_nms_class_agnostic(scores, labels, bboxes, nms_thresh, nms_iou_type, topk=10):
    # 先排除 x1>=x2,y1>=y2的
...
def multiclass_nms_class_aware(scores, labels, bboxes, nms_thresh, nms_iou_type, num_classes, topk=10):
    # 先排除 x1>=x2,y1>=y2的
    inds = np.where((bboxes[:, 2] >= bboxes[:, 0] + 1) * (bboxes[:, 3] >= bboxes[:, 1] + 1))[0]
    scores = scores[inds]
    labels = labels[inds]

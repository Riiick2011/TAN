...
class UCF_JHMDB_Evaluator(object):  # 评估器类
    def __init__(self,
        ...
        self.infer_dir = os.path.join(os.path.abspath('.'), args.infer_dir)  # 推断结果根目录的绝对路径

        self.track_mode = args.track_mode  # 是否采用track_mode模式
        # 计算最远跟踪距离
        self.track_maxdis = (self.len_clip//len(self.sampling_rate))*sum(self.sampling_rate)


        # 只用于frame mAP计算
        self.gt_folder = os.path.join(os.path.abspath('.'), gt_folder)  # gt的绝对路径
        self.map_path = os.path.join(os.path.abspath('.'), args.map_path)  # 保存评估结果的绝对路径
        ...
        if metric == 'fmap':
            self.testset = UCF_JHMDB_Dataset(
                ...
                sampling_rate=self.sampling_rate)  # 构建trimmed测试集
            ...
            self.testset = UCF_JHMDB_VIDEO_Dataset(
                ...
                transform=transform,
                len_clip=self.len_clip,
                sampling_rate=self.sampling_rate)   # 构建untrimmed测试集
            self.num_classes = self.testset.num_classes
    ...
    def evaluate_frame_map(self, model, epoch=1, show_pr_curve=False):  # 评估frame mAP
        ...
        if not os.path.exists(inference_dir):  # 完整路径
            ...
            for batch_idx, (batch_img_name, batch_video_clip, batch_time_difs, batch_key_frame_target, _) \
                ...
                with torch.no_grad():  # 表明当前计算不需要反向传播，使用之后，强制后边的内容不进行计算图的构建
                    ...
                    if self.track_mode:  # 所有批次大小自动重置为1
                        # 读取det_bank并生成tensor格式的track，并且将检测结果保存到bank中

                        img_name_split = batch_img_name[0].split('/')
                        frame_id = img_name_split[-1][:5]
                        video_name = img_name_split[-2]
                        if video_name != video_name_last_det:  # 如果是新视频，则无作为依据
                            det_bank = []
                        # 读取det_bank并生成tensor格式的track
                        if len(det_bank):
                            track = np.concatenate(det_bank, axis=0)
                            track[:, 0] = track[:, 0] * -1 + int(frame_id)
                            track = track[(track[:, 0] < self.track_maxdis)]
                            track = [torch.tensor(track, dtype=torch.float32).to(model.device)]  # 也有可能过滤后是空tensor
                        else:
                            track = [torch.tensor([], dtype=torch.float32).to(model.device)]

                        batch_scores, batch_labels, batch_bboxes = model(batch_video_clip, batch_time_difs, track)

                        # 如果有检测输出的话，则将该帧检测结果存储在det_bank中
                        if len(batch_scores[0]):
                            det_array = np.concatenate((np.ones((len(batch_scores[0]), 1)) * int(frame_id),
                                                        batch_labels[0].reshape(-1, 1),
                                                        batch_scores[0].reshape(-1, 1),
                                                        batch_bboxes[0]), axis=1)
                            det_bank.append(det_array)

                        video_name_last_det = video_name  # 保存当前检测的视频名称

                    else:
                        batch_scores, batch_labels, batch_bboxes = model(batch_video_clip, batch_time_difs)
                    ...
                    for bi in range(len(batch_scores)):
                        ...
                        target = batch_key_frame_target[bi]
                ...
                if not self.frame_det_dir:  # 如果不存在视频数据集的帧级检测结果，则重新进行推断
                    ...
                    for video_id, video_name in enumerate(self.videolist):
                        ...
                        if self.track_mode:
                            video_name_last_det = ''
                        dets_video = {}
                        for batch_idx, (batch_img_name, batch_video_clip, batch_time_difs, batch_key_frame_target,
                                        _) in enumerate(self.testloader):
                            # to device
                            batch_video_clip = batch_video_clip.to(model.device)

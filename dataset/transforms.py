...
class Augmentation(object):
    ...
    def __call__(self, video_clip, target_list):
        ...
        # video_clip是列表表示的视频片段，每一项是一帧增强后图像对应的tensor；
        # target_tensor_list中的target是一个tensor对应一帧的标注，可能为空tensor;
        # target是增强后的真实标注框(丢弃了增强过程中不合理的真实标注框图)，百分比两点式
        return video_clip, target_tensor_list
...
class BaseTransform(object):
    ...
    def __call__(self, video_clip, target_list=None, normalize=True):
        ...
        return video_clip, target_tensor_list  # video_clip是一个列表，每一项对应一帧图像的tensor
        # target_tensor_list中的target是一个tensor对应一帧的标注，可能为空tensor
